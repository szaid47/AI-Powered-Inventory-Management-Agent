...
def send_email(subject: str, body: str) -> str:
    ...
    try:
        host, port, user, password, to_email = (
            os.getenv("EMAIL_HOST"), int(os.getenv("EMAIL_PORT")), os.getenv("EMAIL_USER"),
            os.getenv("EMAIL_PASS"), os.getenv("EMAIL_TO")
        )
...
def get_inventory_data(product_name: str) -> str:
    """Use this tool to get raw inventory data for a specific product."""
    ...
    conn.close()
    if result:
        return f"Data for {result[0]}: Stock={result[1]}, Demand={result[2]}/day, Lead Time={result[3]} days."
    return f"Product '{product_name}' not found."

@tool
def get_all_inventory() -> str:
    """Use this tool to get a summary of all products in the inventory."""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def calculate_reorder_quantity(product_name: str) -> str:
    """Calculates a reorder recommendation for a product. Use this as the primary tool for reorder questions."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT product_name, current_stock, average_demand, lead_time FROM inventory WHERE product_name LIKE ?', (f'%{product_name}%',))
    result = cursor.fetchone()
    ...
    product, stock, demand, lead_time = result
    if demand <= 0:
        return f"✅ {product}: No reorder needed (zero demand)."
    
    days_remaining = stock / demand
    if days_remaining <= lead_time:
        recommended_qty = int(demand * lead_time * 1.5) # Add a 50% safety stock buffer
        return (f"⚠️ **ACTION REQUIRED for {product.title()}**: Stock is low ({stock} units, ~{days_remaining:.1f} days remaining). "
                f"Recommend reordering at least **{recommended_qty} units** immediately.")
    else:
        return (f"✅ **OK for {product.title()}**: Stock level is sufficient ({stock} units, ~{days_remaining:.1f} days remaining). "
                f"No immediate reorder needed.")

# --- Agent Setup ---
system_prompt = """You are an expert inventory management agent. Your goal is to provide clear, actionable reorder advice.

**Core Workflow:**
1.  When a user asks if they should reorder a product (e.g., "should I reorder toothpaste?"), you **MUST** use the `calculate_reorder_quantity` tool. This is your primary tool for analysis.
2.  For general questions about inventory levels (e.g., "how much soap is left?" or "show me everything"), use `get_inventory_data` or `get_all_inventory`.
3.  Base your final answer *exclusively* on the output from the tool you use. Do not add information not returned by the tool.
4.  Present the tool's output to the user in a clear, easy-to-read format.
"""
...
tools = [calculate_reorder_quantity, get_inventory_data, get_all_inventory]
